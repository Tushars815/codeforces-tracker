...
const getQuestionStats = (QuestionMap: QuestionMapType) => {
  ...
  for (const [, value] of Object.entries(QuestionMap)) {
    totalSubmission += value.solved
      ? value.incorrectSubmissions + 1
      : value.incorrectSubmissions;
  ...
  questionData.AverageAttempts = (
    totalSubmission / questionData.QuestionsSolved
  ).toFixed(2);
  questionData.QuestionsAttempted = Object.entries(QuestionMap).length;
  questionData.QuestionsUnsolved =
    questionData.QuestionsAttempted - questionData.QuestionsSolved;
  return questionData;
};
